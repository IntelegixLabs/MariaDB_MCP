...
class MariaDBServer:
    ...
    def _is_safe_identifier(name: Optional[str]) -> bool:
        """
        Validates database/table identifiers to prevent injection.
        Allows only alphanumerics and underscores, at least 1 character.
        """
    ...
    async def _database_exists(self, database_name: str) -> bool:
        ...
        if not self._is_safe_identifier(database_name):
    ...
    async def _table_exists(self, database_name: str, table_name: str) -> bool:
        ...
        if not self._is_safe_identifier(database_name) or not self._is_safe_identifier(table_name):
    ...
    async def execute_sql(self, sql_query: str, database_name: str, parameters: Optional[List[Any]] = None) -> List[Dict[str, Any]]:
        ...
        if database_name and not self._is_safe_identifier(database_name):
    ...
    async def create_vector_store_tool(self,
        ...
        if not self._is_safe_identifier(vector_store_name):
        ...
        # --- SQL Queries for Vector Store Table and Metadata Creation ---
        schema_query = f"""
        ...
        """
        # Meta table to persist store configuration
        meta_table = "_mcp_vector_meta"
        meta_table_query = f"""
        CREATE TABLE IF NOT EXISTS `{meta_table}` (
            store_name VARCHAR(255) NOT NULL PRIMARY KEY,
            model_name VARCHAR(255),
            embedding_dimension INT NOT NULL,
            distance_function VARCHAR(16) NOT NULL
        );
        """
        ...
        try:
            ...
            await self._execute_query(meta_table_query, database=database_name)
            # Upsert metadata for this store
            upsert_meta = f"""
            INSERT INTO `{database_name}`.`{meta_table}` (store_name, model_name, embedding_dimension, distance_function)
            VALUES (%s, %s, %s, %s)
            ...
            """
            await self._execute_query(upsert_meta, params=(vector_store_name, model_name or '', embedding_length, processed_distance_function_sql), database=database_name)
    ...
    async def search_vector_store(self, user_query: str, database_name: str, vector_store_name: str, k: int = 7) -> list:
        ...
        # Determine distance function from metadata (default COSINE if not found)
        distance_sql = "COSINE"
        try:
            meta_row = await self._execute_query(
                f"SELECT distance_function FROM `{database_name}`.`_mcp_vector_meta` WHERE store_name = %s",
                params=(vector_store_name,),
                database=database_name
            )
            if meta_row and isinstance(meta_row, list) and 'distance_function' in meta_row[0]:
                df = str(meta_row[0]['distance_function']).upper()
                if df in ("COSINE", "EUCLIDEAN"):
                    distance_sql = df
        except Exception:
            # If meta not available, fall back silently to COSINE
            pass

        # Generate embedding for the query
        embedding = await embedding_service.embed(user_query)
        emb_str = json.dumps(embedding)
        # Prepare the search query with selected distance function
        distance_func = f"VEC_DISTANCE_{distance_sql}"
        search_query = f"""
            SELECT 
                document,
                metadata,
    ...
    def register_tools(self):
        ...
        @self.mcp.tool
        async def external_execute_sql(db_type: str, dsn: str, sql_query: str, parameters: Optional[List[Any]] = None) -> List[Dict[str, Any]]:
            """
            Execute a read-only SQL query against an external database.
            - db_type: "postgres" or "sqlite"
            - dsn: connection string (e.g., postgres://user:pass@host:port/db or path to sqlite file)
            - sql_query: must start with SELECT/SHOW/DESC/DESCRIBE
            - parameters: optional list mapped to placeholders (%s for Postgres, ? for SQLite)
            """
            allowed_prefixes = ('SELECT', 'SHOW', 'DESC', 'DESCRIBE')
            query_upper = re.sub(r'/\*.*?\*/', '', re.sub(r'--.*?$', '', sql_query, flags=re.MULTILINE), flags=re.DOTALL).strip().upper()
            if not any(query_upper.startswith(p) for p in allowed_prefixes):
                raise PermissionError("Operation forbidden: read-only queries only.")
            ...
            if db_type == "postgres":
                ...
                try:
                    rows = await conn.fetch(sql_query.replace('%s', '$1') if parameters and sql_query.count('%s') == 1 else sql_query, *(parameters or ()))
                    # convert Record -> dict
                ...
                async with aiosqlite.connect(dsn) as conn:
                    ...
                    async with conn.execute(sql_query, parameters or []) as cursor:
